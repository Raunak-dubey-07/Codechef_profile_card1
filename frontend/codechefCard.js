...
const getUser = async (Username) => {
    try {
        ...
        g.innerHTML = "";
        const res = await fetch("https://codechef-api-beta.vercel.app/profile/" + Username);
        if (!res.ok) {
            throw new Error(`Error: ${res.status} - User not found`);
        }
        const data = await res.json();
        console.log(data);
        let arr = data.lastContests;
        let n = arr.length;
        ...
        const card = `
            <div class="container">
                <div class="profileInfo">
                    <img class="profilimg" src="${data.avatar}">
                    <h4>${data.name}</h4>
...
//         // ✅ FETCH FROM LOCAL API (NOT CodeChef directly)
...
//                     <div class="countryinfo">
//                         <img src="${data.countryFlag}" width="20">
//                         <h5>${data.country}</h5>
//                     </div>
